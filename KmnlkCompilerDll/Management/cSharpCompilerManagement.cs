...
{
    public class cSharpCompilerManagement : IcSharpCompilerOperations, IValidationOperations
    {
        private ILog logger;
        ...
        public cSharpCompilerManagement(ILog logger,string tempFiles,string version="v4")
        {
            this.tempFiles = tempFiles;
            this.logger = logger;
            this.provOptions =new Dictionary<string, string>();
            provOptions.Add("CompilerVersion", version);
            //provider = CodeDomProvider.CreateProvider(modConstant.LANGUAGE_CSharp_NAME, provOptions);
            provider = CodeDomProvider.CreateProvider(modConstant.LANGUAGE_CSharp_NAME);
        ...
        public string checkFileCode(string dataFolderPath,clsFileCode file)
        {
            ...
            try
            {
                ...
                {
                    //sb.Append(String.Format("Source {0} built into {1} successfully.", file.codes.ToArray(), results.PathToAssembly));
        ...
        public string generateFileCodeEXE(string dataFolderPath, clsFileCode file, string path)
        {
            ...
            try
            {
                ...
                // Generate an executable instead of 
                // a class library.
                parameters.GenerateExecutable = true; //EXE
                // Save the assembly as a physical file.
                parameters.GenerateInMemory = false;
